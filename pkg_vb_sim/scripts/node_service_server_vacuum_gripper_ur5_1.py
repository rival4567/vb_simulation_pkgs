...
class VacuumGripper():
    ...
    def __init__(self):
        ...
        param_config_vacuum_gripper = rospy.get_param(
            'config_vacuum_gripper_ur5_1')
        ...
        self._attachable_object_delimiter = param_config_vacuum_gripper[
            'attachable_object_delimiter']
        ...
        rospy.loginfo(
            '\033[94m' + " >>> Vacuum Gripper init done." + '\033[0m')
    ...
    def callback_service_on_request(self, req):
        ...
        self._flag_plugin_in_use = rospy.get_param(
            "vacuum_gripper_plugin_in_usage")

        rospy.loginfo('\033[94m' + " >>> Vacuum Gripper Activate: {}".format(
            req.activate_vacuum_gripper) + '\033[0m')
        rospy.loginfo(
            '\033[94m' + " >>> Vacuum Gripper Flag Pickable: {}".format(self._flag_pickable) + '\033[0m')
        rospy.loginfo('\033[94m' + " >>> Vacuum Gripper Plugin in Use: {}".format(
            self._flag_plugin_in_use) + '\033[0m')
    ...
    def callback_topic_subscription(self, rx_msg):
        ...
        if ((self._count > 1) and (number_models == 0)):
            ...
            for i in range(0, number_models):
                ...
                lst_name_model = name_model.split(
                    self._attachable_object_delimiter)

                if(lst_name_model[0] == self._attachable_object_prefix):
                    rospy.loginfo('\033[94m' + ">>> [ur5_1] Vacuum Gripper: Pickable object found {}. Pickable: {}".format(
                        name_model, self._flag_pickable) + '\033[0m')
...
def main():
    ...
    s = rospy.Service('/eyrc/vb/ur5/activate_vacuum_gripper/ur5_1',
                      vacuumGripper, ur5_vacuum_gripper.callback_service_on_request)
    rospy.loginfo(
        '\033[94m' + " >>> Vacuum Gripper Activation Service Ready." + '\033[0m')

    rospy.Subscriber(ur5_vacuum_gripper._logical_camera_topic_name,
                     LogicalCameraImage, ur5_vacuum_gripper.callback_topic_subscription)
